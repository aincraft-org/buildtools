...
### Current notes
V1 command surface is `/masonry` (pos1/pos2, replace, fill, survival_fill, copy, paste, undo/redo, blueprint save/load/list/delete). Defaults: `interactionDistance=6`, `selectionExtent=64`, `maxOperationBlocks=32768`. Held-block extension is a separate input path: hold the ordinary `minecraft:brick` item in the main hand and aim at the block to extend; the preview appears while the token is held, right-click places the current preview, sneak-scroll extends or retracts length, normal scroll changes hotbar slots, and Shift is only needed while scrolling to change the length. The action uses the normal survival/undo lifecycle and charges matching placeable blocks from the player's inventory. Top-face clicks extend horizontally along the player's aim; horizontal side-face clicks extend out from the clicked face. Live…
