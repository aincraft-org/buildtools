...
### Current notes
V1 command surface is `/masonry` (pos1/pos2, replace, fill, survival_fill, copy, paste, undo/redo, blueprint save/load/list/delete). Defaults: `interactionDistance=6`, `selectionExtent=64`, `maxOperationBlocks=32768`. Held-block extension is a separate input path: hold the ordinary `minecraft:brick` item in the main hand and aim at the block to extend; the preview appears while the token is held, right-click grows one row, sneak-scroll extends or retracts length, normal scroll changes hotbar slots, and sneak+right-click commits through the normal survival/undo lifecycle, charging matching placeable blocks from the player's inventory. Top-face clicks extend horizontally along the player's aim; horizontal side-face clicks extend out from the clicked face. Live Paper playthrough is not required to mark Current done. The gadget (2026-08-24 design) adds a shift-click item UI over th…
